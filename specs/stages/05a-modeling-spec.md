...
**Decision (2025-12-12):** This analysis is **prediction-primary with interpretability**.
...
**Implications:**
- Index reduction uses correlation/VIF pruning (not PCA) to preserve interpretability
- Cross-validation assesses predictive generalization, not just in-sample fit
- Model coefficients are reported but interpreted cautiously
- Multicollinearity among predictors is less concerning than it would be for pure inference
...
**Note:** Model validation (AR1 autocorrelation validation, cross-validation) is specified separately in `05b-validation-spec.md`.
