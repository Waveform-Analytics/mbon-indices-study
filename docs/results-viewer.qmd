...
| Metric | Top 3 Indices | What these indices measure |
|--------|---------------|---------------------------|
| **fish_activity** | SNRt, BGNt, EVNtCount | Signal-to-noise, background noise levels, acoustic event counts |
| **fish_richness** | EVNtCount, aROI, LFC | Event counts, activity regions, low-frequency content |
| **fish_presence** | LFC, Hf, AGI | Low-frequency content, spectral entropy, acoustic gradients |
| **dolphin_burst_pulse** | EPS, ACTspMean, H_gamma | Spectral entropy, activity intensity, information content |
| **dolphin_echolocation** | ACTspCount, ACTspFract, EPS | Activity counts and fractions, spectral entropy |
| **dolphin_whistle** | ZCR, EVNspMean, BGNf | Zero-crossing rate, spectral events, frequency-band noise |
| **dolphin_activity** | ACTspCount, ACTspFract, EPS | Activity counts and fractions, spectral entropy |
| **dolphin_presence** | ACTspFract, HFC, EVNspCount | Activity fraction, high-frequency content, spectral events |
| **vessel_presence** | HFC, aROI, nROI | High-frequency content, activity regions |
...
- **Vessel presence has huge effects.** The index nROI (number of regions of interest) alone changes vessel detection probability by +45 percentage points. Multiple indices exceed the ±10 pp threshold. This explains the excellent AUC — the signal is genuinely strong.

- **Fish presence has moderate effects.** BioEnergy (−36 pp), ZCR (+19 pp), and ROU (+17 pp) exceed the threshold. There's enough signal to make useful predictions.

- **Dolphin presence effects are tiny.** The largest is under 7 percentage points. Even though many indices are "statistically significant," none makes a meaningful practical difference on its own. This is why the dolphin models have lower AUC — the available signal is weak.

**The insight:** Statistical significance is necessary but not sufficient. Focus on indices with large effect sizes for practical applications.
...
## Appendix C: Per-Station Index Significance {#appendix-bystation}
...
To answer this, we fit separate GAMM models for each station and extracted which indices were statistically significant (p < 0.05) at each location.

### Indices That Matter Everywhere

These indices were significant at **all three stations** — suggesting robust, location-independent relationships:

| Metric | Indices Significant at All Stations |
|--------|-------------------------------------|
| **fish_presence** | EVNtMean, LFC, NDSI, VARf, aROI, rBA |
| **vessel_presence** | LFC, MFC, aROI, nROI |
| **dolphin_presence** | ACI, LEQt, TFSD, aROI |

**What these indices have in common:** Most capture broad acoustic characteristics — aROI (area of regions of interest) appears in all three, suggesting that the overall "acoustic activity" level is a universal predictor. LFC (low-frequency content) is important for both fish and vessels, which makes sense given both produce low-frequency sounds.

### How Many Indices Are Significant by Station?

| Metric | 9M | 14M | 37M | Notes |
|--------|-----|------|------|-------|
| vessel_presence | 19 | 21 | 23 | Consistent across stations |
| fish_presence | 19 | 20 | 21 | Consistent across stations |
| fish_richness | 13 | 14 | 13 | Consistent across stations |
| dolphin_presence | 14 | 21 | 14 | More indices matter at 14M |
| dolphin_activity | 9 | 10 | 21 | **37M has 2× more** |
| dolphin_echolocation | 10 | 14 | 18 | More indices at deeper stations |
| dolphin_burst_pulse | 11 | 11 | 17 | More indices at 37M |
| dolphin_whistle | 8 | **0** | 6 | Nothing significant at 14M |
| fish_activity | N/A | 17 | 17 | 9M model failed |

**Patterns:**

- **Fish and vessel metrics** are consistent — similar numbers of significant indices at all stations
- **Dolphin metrics vary by station** — 37M (the deepest site) tends to have more significant indices, possibly because dolphins are more acoustically active in deeper water
- **dolphin_whistle at 14M** is an outlier — zero significant indices, compared to 6-8 at other stations. This could indicate station-specific dolphin behavior or acoustic conditions that make whistle detection harder at 14M

### Station-Specific Effects

Many indices show effects at only 1-2 stations. For example:

- **BGNt** (background noise temporal) predicts dolphin_activity at 37M but not at shallower stations
- **BioEnergy** predicts vessel_presence at 14M and 37M but not at 9M
- **NBPEAKS** (number of spectral peaks) predicts dolphin_activity at 9M but not elsewhere

This suggests that while some acoustic characteristics are universally informative, others depend on local acoustic conditions, depth, or species behavior at each site.

### Downloads

- [Per-Station Index Significance (CSV)](tables/bystation_significance_summary.csv) — Wide format with EDF and p-values for each station
